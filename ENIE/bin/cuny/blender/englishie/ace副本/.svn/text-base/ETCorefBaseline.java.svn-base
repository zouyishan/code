...
public class ETCorefBaseline {
	...
	ArrayList constructEvent1 (AceDocument aceDoc,String docId){
		int aceEventNo = 1;
		ArrayList newEvents = new ArrayList();
		for (int i=0; i<aceDoc.events.size(); i++){
			AceEvent event = aceDoc.events.get(i);
			AceEvent newEvent = findEventCluster(event.subtype,newEvents);
			if (newEvent==null){
				String eventId = docId + "-EV" + aceEventNo;
				newEvent = new AceEvent(eventId,event.type,event.subtype,
						event.modality,event.polarity,event.genericity,event.tense);
				newEvents.add(newEvent);
				aceEventNo++;
			}
...
*/		//	bw1.close();
		...
		//	bw3.close();
			...
			//	bw3.write("folds=" + Integer.toString(fold) + "\n");
				...
				ETCorefBaseline et = new ETCorefBaseline();
				/*
				if (trainCorefModel) {
					et.trainCorefModel ();
				}*/
	...
	private static void evaluateEventTagger (ETCorefBaseline et)  throws IOException {
