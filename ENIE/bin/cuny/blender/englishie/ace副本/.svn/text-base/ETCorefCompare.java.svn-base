...
public class ETCorefCompare {
	...
	ArrayList constructEvent2 (AceDocument aceDoc,String docId){
		int aceEventNo = 1;
		ArrayList<AceEvent> newEvents = new ArrayList<AceEvent>();
		for (int i=0; i<aceDoc.events.size(); i++){
			AceEvent event = aceDoc.events.get(i);
			AceEvent oldEvent = null;
			for (int j=0;j<newEvents.size();j++){
				if (newEvents.get(j).subtype.equals(event.subtype)){
					oldEvent = newEvents.get(j);
					break;
				}
			}
			if (oldEvent!=null){
				oldEvent.mentions.addAll(event.mentions);
			}
			else{
				newEvents.add(event);
			}
	...
	static final String corefFeatureFileName = aceModels + "corefFeatureFile1.log";
	static final String corefModelFileName = aceModels + "corefModel1.log";
	...
	static String mucFileName1 = home +"coref_muc_agg";
	static String bcubeFileName1 = home +"coref_bcube_agg";
	static String eacfFileName1 = home +"coref_eacf_agg";
	
	static String mucFileName2 = home +"coref_muc_graph";
	static String bcubeFileName2 = home +"coref_bcube_graph";
	static String eacfFileName2 = home +"coref_eacf_graph";
	...
	static String logfile = home+"coref1.log";
	...
	public static void main (String[] args) throws IOException {
		...
		for (int runs = 0; runs < 1; runs++) {
			...
			for (int fold = 0; fold < 10; fold++) {
				...
				ETCorefCompare et = new ETCorefCompare();
		
				ETGraphCorefV2 graph = new ETGraphCorefV2();
				if (trainCorefModel) {
					et.trainCorefModel ();
					graph.trainCorefModel();
				}
				if (evaluate) {
					...
					//bw1.write("MUC\n");
					bw11.write(Double.toString(ee1.precision)+"\t"+Double.toString(ee1.recall)+"\t"+Double.toString(ee1.F)+"\n");
					//bw2.write("BCube\n");
					BCubeCorefEval ee2 = new BCubeCorefEval();
					ee2.evalCoref(fileListTest);
					bw21.write(Double.toString(ee2.precision)+"\t"+Double.toString(ee2.recall)+"\t"+Double.toString(ee2.F)+"\n");
					System.out.println(Double.toString(ee2.N) + "\t"+Double.toString(ee2.N*ee2.precision)+"\t"+Double.toString(ee2.N*ee2.recall));
					/*FileWriter log = new FileWriter(logfile);
					log.flush();
					log.write(ee2.log.toString());
					log.close();*/
					//bw3.write("CEAF\n");
					CEAFCorefEval1 ee3 = new CEAFCorefEval1();
					ee3.evalCoref(fileListTest);
					bw31.write(Double.toString(ee3.precision)+"\t"+Double.toString(ee3.recall)+"\t"+Double.toString(ee3.F)+"\n");
				
					graph.loadAllModels (aceModels);
					graph.evaluateEventTagger(graph,0);
					
					ee1 = new MUCCorefEval();
					ee1.evalCoref(fileListTest);
				
					bw12.write(Double.toString(ee1.precision) + "\t"
						+ Double.toString(ee1.recall) + "\t"
						+ Double.toString(ee1.F) + "\n");
				// bw.write("BCube\n");*/
					ee2 = new BCubeCorefEval();
					ee2.evalCoref(fileListTest);
					bw22.write(Double.toString(ee2.precision) + "\t"
						+ Double.toString(ee2.recall) + "\t"
						+ Double.toString(ee2.F) + "\n");
					ee3 = new CEAFCorefEval1();
					ee3.evalCoref(fileListTest);
					bw32.write(Double.toString(ee3.precision) + "\t"
	...
	private static void evaluateEventTagger (ETCorefCompare et)  throws IOException {
	...
	public void loadAllModels (String modelDir) throws IOException {
		...
		corefModel = loadClassifierModel(modelDir + "corefModel1.log");
	}
