...
public class RelationTagger {
	...
	static String aceModels = home +"model/relation/";
	...
	public void train(String fileList, int pass) throws IOException {
		...
		while ((currentDocPath = reader.readLine()) != null) {
			...
			if (pass == 0) {
				trainRelationClassifier(doc, aceDoc, currentDocPath);
			}
		}
	...
	private void trainRelationClassifier(Document doc, AceDocument aceDoc,
		...
		for (int i = 0; i < sentences.size(); i++) {
			...
			Vector<AceEntityMention> ms = findEntityInSentence(doc,aceDoc,s);
			if (ms.size()==0)	
				continue;
			...
			for (int j = 0; j<ms.size();j++){
				...
				for (int k=0;k<ms.size();k++){
					...
					FeatureSet f = relationFeatures(doc,aceDoc,s.span(),m1,m2, structure,allEntityMentionsList);
	...
	private FeatureSet relationFeatures(Document doc, AceDocument aceDoc, Span sentence, AceEntityMention m1,AceEntityMention m2,int structure,ArrayList<AceEntityMention> allEntityMentionsList){
		...
		if (m1.extent.within(m2.extent)||m2.extent.within(m1.extent)||m1.extent.overlap(m2.extent)){
			...
			if (m1.extent.end()<m2.extent.start()){
				...
				str1 = doc.text(new Span(sentence.start(),m1.extent.start()));
				str2 = doc.text(new Span(m2.extent.end()+1,sentence.end() ));
			}
			else {
				s=new Span(m2.extent.end()+1,m1.extent.start());
				str1 = doc.text(new Span(sentence.start(),m2.extent.start() ));
				str2 = doc.text(new Span(m1.extent.end()+1,sentence.end() ));
			}
			...
			while (s1.hasMoreTokens()) {
				j++;
				arrList1.add(s1.nextToken());
			}
			String [] arr1 = new String[arrList1.size()];
			arrList1.toArray(arr1);
			...
			while (s2.hasMoreTokens()) {
				j++;
				arrList2.add(s2.nextToken());
			}
			String [] arr2 = new String[arrList2.size()];
			arrList2.toArray(arr2);
			
			if (arr1.length>=1)
				f.addFV("BM1F",arr1[arr1.length-1]);
			if (arr1.length>=2) 
				f.addFV("BM1L",arr1[arr1.length-2]);
			if (arr2.length>=1)
				f.addFV("AM2F",arr2[0]);
			if (arr2.length>=2) 
				f.addFV("AM2L",arr2[1]);
			...
			while (s3.hasMoreTokens()) {
				j++;
				arrList3.add(s3.nextToken());
			}
			String [] arr3 = new String[arrList3.size()];
			arrList3.toArray(arr3);
			...
			if (arr3.length==1){
				...
				f.addFV("WBF",arr3[0]);
				f.addFV("WBL",arr3[arr3.length-1]);
			}
			else if (arr3.length>2){
				f.addFV("WBF",arr3[0]);
				f.addFV("WBL2",arr3[arr3.length-1]);
				f.addFV("WBL1",arr3[arr3.length-2]);
			}
			
			/*if (arr3.length>=3){
				String internal ="";
				for (int k=1;k<=arr3.length-2;k++)
					internal+=arr3[k];
				f.addFV("WBO",internal);
			}*/
		}
	...
	public static void main(String[] args) throws IOException {
		...
		RelationTagger tagger = new RelationTagger();
	...
	public void Tag1(Document doc, AceDocument aceDoc, String currentDocPath) throws IOException{
		int aceRelationNo = 1;
		...
		for (int i=0; i<sentences.size();i++){
			...
			for (int j = 0; j<ms.size();j++){
				...
				for (int k=0;k<ms.size();k++){
					...
					AceEntityMention m2 = ms.get(k);
					int structure = checkPosStructure(m1,m2, allEntityMentionsList);
					FeatureSet f = relationFeatures(doc,aceDoc,ann.span(),m1,m2, structure,allEntityMentionsList);
					
					double relationProb = relationIdenModel.eval(f.toArray())[relationIdenModel.getIndex("yes")];
					...
					if (relationProb>=MIN_RELATION_IDEN_THRESHOLD && (structure==1||
						...
						if (rsubtypeProb>=MIN_RELATION_THRESHOLD){
							...
							AceRelation r = new AceRelation (rId, (String)subtypeTable.get(rsubtype), rsubtype, "EXPLICIT",m1.entity, m2.entity);
							String rmId = rId+"-1";
							AceRelationMention rm =new AceRelationMention (rmId, m1, m2, doc);
		...
		for (int i=0; i<sentences.size();i++){
			...
			for (int j = 0; j<ms.size()-1;j++){
				...
				for (int k=j+1;k<ms.size();k++){
					...
					FeatureSet f1 = relationFeatures(doc,aceDoc,ann.span(),m1,m2, structure1,allEntityMentionsList);
					
					int structure2 = checkPosStructure(m2,m1, allEntityMentionsList);
					FeatureSet f2 = relationFeatures(doc,aceDoc,ann.span(),m2,m1, structure2,allEntityMentionsList);
