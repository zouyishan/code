...
import cuny.blender.englishie.nlp.refres.CorefFilter;
...
import cuny.blender.englishie.nlp.zoner.SpeechSplitter;
		...
		for (int j = 0; j < actions.length; j++) {
			...
			if (colon > 0 && colon < action.length() - 1) {
				...
				for (int i = 0; i < zones.size(); i++) {
					//System.out.println("processing zone "+zones.get(i)+":"+zoneScript);
					//System.out.println(doc.text((Annotation) zones.get(i)));
				...
				if (JetTest.nameTagger2 == null)
					Console.println("Error:  no name model loaded");
				else
					JetTest.nameTagger2.tag(doc, span);
