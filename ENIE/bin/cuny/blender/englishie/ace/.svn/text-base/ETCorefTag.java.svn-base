...
public class ETCorefTag {
	...
	static double COREF_THRESHOLD = 0.3;
	...
	//static final String home ="/home/zheng/Application/Coref/ex1/ACE05/";
		...
		for (int i=0; i<events.size(); i++) {
			...
			for (int iEvent = 0; iEvent < systemEvents.size(); iEvent++) {
				...
				if (!priorEvent.subtype.equals(newEvent.subtype)) continue;
					// if same type/subtype, generate training example
					// (with outcome = whether it belongs to this systemEvent)
				Datum d = corefFeatures (priorEvent, newEvent,anchor,doc,3);
				d.setOutcome((iEvent == systemEventIndex) ? "merge" : "dontMerge");
				corefFeatureWriter.println(d.toString());
			}
				// if it should be folded in, do so; else create new event
		...
		for (int i=0; i<events.size(); i++) {
			...
			for (int j=newEvents.size()-1; j>=0; j--) {
				...
				if (event.type.equals(newEvent.type) &&
				  ...
				  Datum d = corefFeatures (newEvent, event, anchor,doc,3);
	...
	static final String fileListTrain = home + "train.txt";
	...
	static final String fileListTest = home + "dev.txt";
	static final String fileListDev =home+ "dev.txt";
	// ---- for eval
	...
	public static void main (String[] args) throws IOException {
		...
		reader = new BufferedReader(new FileReader(fileListTest));
		while ((currentDocPath = reader.readLine()) != null) {
			v.remove(currentDocPath);
		}
		reader.close();
		
		reader = new BufferedReader(new FileReader(fileListDev));
		while ((currentDocPath = reader.readLine()) != null) {
			v.remove(currentDocPath);
		}
		reader.close();
		
		FileWriter fw = new FileWriter(fileListTrain);
		for (int i=0;i<v.size();i++)
			fw.write(v.get(i)+"\n");
		fw.close();
		*/
		...
		if (evaluate) {
			...
			//for (COREF_THRESHOLD=0.0;COREF_THRESHOLD<0.98;COREF_THRESHOLD=COREF_THRESHOLD+0.05){
	...
	private static void evaluateEventTagger (ETCorefTag et)  throws IOException {
