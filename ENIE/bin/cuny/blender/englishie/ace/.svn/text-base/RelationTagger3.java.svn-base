...
public class RelationTagger3 {
	...
	static String aceModels = home +"model/relation5/";
	...
	private void trainRelationClassifier(Document doc, AceDocument aceDoc,
		...
		for (int i = 0; i < sentences.size(); i++) {
			...
			/*if (i==31){
				for (AceEntityMention m:ms)
					System.out.println(m.headText);
			}*/
			if (s.length()>500)
				continue;
			System.out.println("Sentence "+(i+1)+"/"+(sentences.size()+1));
		...
		/*	List<TypedDependency> tdl = StanfordParser.parse2(s);
			...
			for (int j = 0; j<ms.size();j++){
				...
				for (int k=0;k<ms.size();k++){
					...
					FeatureSet f = relationFeatures(tdl,parse,doc,aceDoc,ann.span(),m1,m2, allEntityMentionsList);
					if (f.size()==0)
						continue;
	...
	private FeatureSet relationFeatures(List<TypedDependency> tdl, String parse, Document doc, AceDocument aceDoc, Span sentence, AceEntityMention m1,AceEntityMention m2,ArrayList<AceEntityMention> allEntityMentionsList){
		...
		/*String path = StanfordParser.getPath(tdl, head1, head2);
		if (path.isEmpty())
		{
			f = new FeatureSet();
			return f;
	...
	public static void main(String[] args) throws Exception {
		...
		RelationTagger3 tagger = new RelationTagger3();
	...
	static double MIN_RELATION_THRESHOLD = 0.35;
			...
			*/
			...
			for (int j = 0; j<ms.size()-1;j++){
				...
				for (int k=j+1;k<ms.size();k++){
					AceEntityMention m2 = ms.get(k);
					FeatureSet f1 = relationFeatures(tdl,parse,doc,aceDoc,ann.span(),m1,m2,allEntityMentionsList);
					if (f1.size()==0)
						continue;
					FeatureSet f2 = relationFeatures(tdl,parse,doc,aceDoc,ann.span(),m2,m1,allEntityMentionsList);
