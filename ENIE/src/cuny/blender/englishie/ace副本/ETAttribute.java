...
public class ETAttribute {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int runs = 0; runs < 10; runs++) {
			...
			for (int fold = 0; fold < 5; fold++) {
				...
				splitCorpus(fold,v);
				ETAttribute et = new ETAttribute();
				...
				if (evaluate) {
					...
					/*FileWriter bw = new FileWriter(logFileName1);
	...
	public  void run() throws IOException {
		...
		processOnlyEvenDocuments = false;
		if (trainPolModels)
			polFeatureWriter = new PrintStream (new FileOutputStream (polFeatureFileName));
		if (trainTenModels)
			tenFeatureWriter = new PrintStream (new FileOutputStream (tenFeatureFileName));
		if (trainGenModels)
			genFeatureWriter = new PrintStream (new FileOutputStream (genFeatureFileName));
		if (trainModModels)
			modFeatureWriter = new PrintStream (new FileOutputStream (modFeatureFileName));
		
		
		if (trainPatterns) {
			if (trainArgModels) {
				argFeatureWriter = new PrintStream (new FileOutputStream (argFeatureFileName));
				roleFeatureWriter = new PrintStream (new FileOutputStream (roleFeatureFileName));
			}
			for (int pass=0; pass<=1; pass++) {
				train (fileListTrain, pass);
				
			}
			report (eventReportFile);
			if (trainArgModels) {
		...
		if (evaluate) {
			...
			loadAllModels(aceModels);
			evaluateEventTagger(this);
	...
	private static void evaluateEventTagger(ETAttribute et) throws IOException {
