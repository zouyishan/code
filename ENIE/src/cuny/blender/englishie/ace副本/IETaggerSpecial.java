...
public class IETaggerSpecial {
	...
	public static void main (String[] args) throws IOException {
		// get arguments 
		System.out.println("Starting English IE toolkit...");
		if (args.length < 4 || args.length > 8) {
			System.out.println ("Ace must take 4 to 8 arguments:");
		...
		while ((currentDocPath = reader.readLine()) != null) {
			...
			if (debug) {
				...
				/*try {
					...
					System.err.println("Generate empty APF due to exceptions in the document");
					//e.printStackTrace();
					*/
					generateEmptyApf(currentDocPath);
				//}
			}
		}
	}
	...
	public static void processFile (String currentDocPath) throws IOException {
		...
		String dir = outputDir+File.separator+currentDocPath;
		int pos = dir.lastIndexOf(File.separator);
		String tagDir = dir.substring(0,pos+1);
		...
		File srcFile = new File(docDir +File.separator+ currentDocPath);
		if (!srcFile.exists()){
			System.out.println("Warning: source file does not exist.");
			return;
		}
		if (srcFile.length()==0){
			System.out.println("Warning: file is empty. This file will be skipped.");
			return;
		}
			
		doc = new ExternalDocument("sgml",  docDir +File.separator+ currentDocPath);
	...
	public static void generateEmptyApf (String currentDocPath) throws IOException {
		System.out.println ("\nProcessing document " + docCount + ": " + currentDocPath);
		String currentDocFileName = (new File(currentDocPath)).getName();
		String currentDocPathBase = removeFileExtension(currentDocPath);
