//Filename:     AceEventCorefAggr.java
//Copyright:    2009
//Author:       Zheng Chen
//Description:  
//	Implement an agglomerative clustering algorithm for event coreference resolution,
//	using perfect or system event mentions
...
public class AceEventCorefAggr {
	...
	private void trainCorefModel (Document doc, AceDocument aceDoc, String docId) {
		...
		for (int i=0; i<events.size(); i++) {
			...
			for (int iEvent = 0; iEvent < systemEvents.size(); iEvent++) {
				...
				Datum d = corefFeatures (priorEvent, newEvent,anchor,doc,2);
	...
	private Datum corefFeatures (AceEvent priorEvent, AceEvent event, String anchor,Document doc, int id ) {
		...
		if (sen1!=-1&&sen2!=-1){
			d.addFV ("sendist", Integer.toString(Math.min(sen2-sen1,9)));
		}
		...
		for (int i=0; i<events.size(); i++) {
			...
			for (int j=newEvents.size()-1; j>=0; j--) {
				...
				if (event.type.equals(newEvent.type) &&
				  ...
				  Datum d = corefFeatures (newEvent, event, anchor,doc,2);
	...
	public static void main (String[] args) throws IOException {
		// get arguments
		System.out.println("Starting agglomerative clustering algorithm for event coreference resolution...");
		System.out.println("Usage example: java -cp /jar/workspace/coref/jet1.5.jar -Xmx1000m -server -DjetHome=$1 -Dthreshold=$2 -Dtype=$3 AceJet.AceEventCorefAggr");
		System.out.println("$1:home path");
		System.out.println("$2:clustering threshold,0.0-1.0, suggested 0.35");
		System.out.println("$3:perfect or system event mentions,perfect,system");
		...
		COREF_THRESHOLD = Double.parseDouble(System.getProperty("threshold"));
		...
		AceEventCorefAggr et = new AceEventCorefAggr();
	...
	private static void evaluateEventTagger (AceEventCorefAggr et)  throws IOException {
