...
public class ETCorefFeaModel {
	...
	private Datum corefFeatures (AceEvent priorEvent, AceEvent event, String anchor,Document doc, int id ) {
		...
		d.addFV ("subtype", event.type+":"+event.subtype);
		// normalized anchor
		d.addFV ("anchor", anchor);
	...
	public static void main (String[] args) throws IOException {
		...
		for (int runs = 0; runs < 1; runs++) {
			...
			for (int fold = 0; fold < 10; fold++) {
				...
				ETCorefFeaModel et = new ETCorefFeaModel();
		
				for (FEATURE_SET=0;FEATURE_SET<2;FEATURE_SET++){
	...
	private static void evaluateEventTagger (ETCorefFeaModel et)  throws IOException {
