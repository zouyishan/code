...
import cuny.blender.englishie.nlp.zoner.SentenceSet;
...
public class LearnRelations {
	...
	public static void main (String[] args) throws IOException {
		...
		String filelist = home+"corpus/ACE05/testfilelist";
		...
		while ((currentDocPath = reader.readLine()) != null) {
			...
			String xmlFile = directory + currentDocPath.replaceFirst(".sgm", ".apf.xml");
			String outputFile = home + "corpus/ACE05/output/relation/"+currentDocPath.replaceFirst(".sgm", ".apf.xml");
			...
			LearnRelations.findRelations2(currentDocPath.replace(".sgm", ""), doc, aceDoc);
			aceDoc.write(new BufferedWriter (new FileWriter (outputFile)),doc);
		}
		
		/*String home = "C:/Users/blender/workspace/blender/";*/
		String testfileList = home+"corpus/ACE05/testfilelist";
		String tagDir = home+"corpus/ACE05/output/relation/";
	...
	void learnFromFileList (String fileList, String textDir, String apfDir) throws IOException {
		...
		while ((currentDoc = reader.readLine()) != null) {
			...
			// read document
			String textFile;
			textFile = textDir + currentDoc.replace(".sgm", "") + textFileSuffix;
			ExternalDocument xdoc = new ExternalDocument("sgml", textFile);
			xdoc.setAllTags(true);
			xdoc.open();
			doc = xdoc;
			// read key file with relation information
			readACErelations (textFile, apfDir + currentDoc.replace(".sgm", "") + apfFileSuffix);
	...
	public static void findRelations2 (String currentDoc, Document d, AceDocument aceDoc) {
		doc = d;
		docName = currentDoc;
		sentences = new SentenceSet(doc);
		// System.out.println ("PartitiveMap: " + Ace.partitiveMap);
   ...
   */
	...
	private static void predictRelations () {
		...
		for (int i=0; i<candidates.size(); i++) {
			...
			RelationPattern match = IETagger.eve.findMatch(rm, maxDistance);
			rm.confidence = IETagger.eve.getMatchConfidence();
	...
	private static void predictRelations2 () {
		relMentionList = new ArrayList<RelationMention>();
		for (int i=0; i<candidates.size(); i++) {
			RelationMention rm = (RelationMention) candidates.get(i);
			if (relationTrace)
			...
			RelationPattern match = eve.findMatch(rm, maxDistance);
			rm.confidence = eve.getMatchConfidence();
			String predictedType = "0", predictedSubtype = "";
			if (match != null) {
				if (relationTrace)
					System.out.println ("Best corpus pattern = " + match.string);
