...
public class RelationTagger2 {
	...
	static TreeMap<String, List<ConnectWords>> clusters = new TreeMap<String, List<ConnectWords>>();
	
	//static HashMap<String, Vector<String>> paraphrases = new Hashmap<String, Vector<String>>();
	...
	private FeatureSet relationFeatures2(String between,String depResult, String parseResult, Document doc, AceDocument aceDoc, Span sentence, AceEntityMention m1,AceEntityMention m2,ArrayList<AceEntityMention> allEntityMentionsList){
		FeatureSet f = new FeatureSet();
	...
	public static void main(String[] args) throws Exception {
		...
		RelationAugment.readClusters(home+"paraphrases_hybrid_ACE05Relation2", clusters);
		...
		for (int i=0; i<sentences.size();i++){
			...
			for (int j = 0; j<ms.size()-1;j++){
				...
				for (int k=j+1;k<ms.size();k++){
					...
					Span  betweenspan;
					if (m1.head.start()<m2.head.start()){
						betweenspan=new Span(m1.head.end()+1,m2.head.start());
						
					}
					else{
						betweenspan=new Span(m2.head.end()+1,m1.head.start());
					}
					
					String str = doc.text(betweenspan).toLowerCase().trim();
					str=str.replace("\n", " ");
					str = str.replaceAll("\\b\\s{2,}\\b", " ");
					
					List<ConnectWords> cws = clusters.get(str);
					List<String> para = new ArrayList<String>();
					para.add(str);
					if (cws!=null){
						for (ConnectWords cw: cws){
							para.add(cw.words);
						}
					}
					
					List<AceRelation> relationCandidates = new ArrayList<AceRelation>();
					for (int l=0;l<para.size();l++){
						FeatureSet f1 = relationFeatures2(para.get(l),depResult,parseResult,doc,aceDoc,ann.span(),m1,m2,allEntityMentionsList);
						if (f1.size()==0)
							continue;
						FeatureSet f2 = relationFeatures2(para.get(l),depResult,parseResult,doc,aceDoc,ann.span(),m2,m1,allEntityMentionsList);
					...
					if (relationCandidates.size()>1){
						for (int m=0;m<relationCandidates.size();m++){ 
							AceRelation r = relationCandidates.get(m);
							AceRelationMention rm = (AceRelationMention) r.mentions.get(0);
							System.out.println(para.get(m)+":"+rm.confidence+":"+r.subtype);
						}
						System.out.println("=====================");
					}
					for (AceRelation r: relationCandidates){
						String subtype = r.subtype;
						if (ts.containsKey(subtype)){
							int count = ts.get(subtype);
							count++;
							ts.put(subtype, count);
						}
						else
							ts.put(subtype,1);
					}
					...
					String subtypeMax = null;
					NavigableSet<String> keys = ts.navigableKeySet();
					for(String key : keys) {
						if(ts.get(key)>max) {
							max = ts.get(key);
							subtypeMax = key;
					...
					double maxCon = Double.MIN_VALUE;
					AceRelation best = null;
					for (AceRelation r: relationCandidates){
						AceRelationMention rm = (AceRelationMention) r.mentions.get(0);
						if (r.subtype.equals(subtypeMax)&&rm.confidence>maxCon){
							maxCon = rm.confidence;
							best = r;
						}
					}
					if (best!=null){
						aceDoc.addRelation(best);
						aceRelationNo++;
					}
				}
			}
