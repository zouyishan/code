...
public class ETCT {
	...
	static double EVENT_PROBABILITY_THRESHOLD = 0.11;// 0.80;
	// args below this prob are never added to an event, and not used
	// in estimating event probability
	static double MIN_ARG_PROBABILITY = 0.14;// 0.10;
	...
	public PrintStream eventFeatureWriter;
	// PrintStream evTypeFeatureWriter;
	public PrintStream argFeatureWriter;
	public PrintStream roleFeatureWriter;
	public PrintStream corefFeatureWriter;
	...
	static final String ace = home + "en_sources/";
	...
	public void train(String fileList, int pass) throws IOException {
		...
		while ((currentDocPath = reader.readLine()) != null) {
			...
			if (processOnlyEvenDocuments && docCount % 2 == 1)
				continue;
			//System.out.println("Processing file " +Integer.toString(docCount)+":"+ currentDocPath);

			String textFile; 
			String xmlFile;
			if (currentDocPath.endsWith(".en.sgm")){
				textFile = home + "boost_en_sgm/"+ currentDocPath;
				xmlFile = home + "boost_ch_en/"
				+ currentDocPath.replaceFirst(".sgm", ".apf.xml");
			}
			else {
				textFile = ace + (useParser ? "perfect-parses/" : "")
				+ currentDocPath;
				xmlFile = ace 
				+ currentDocPath.replaceFirst(".sgm", ".apf.xml");
			}
	...
	private void trainCorefModel(Document doc, AceDocument aceDoc, String docId) {
		...
		for (int i = 0; i < constituents.size(); i++) {
			...
			if (cat == "n" || cat == "v" || cat == "tv" || cat == "ven"
				...
				if (event != null) {
					AceEvent keyEvent = correctEvent(anchorExtent, event,
							events);
					...
					System.out
							.println("systemEventIndex = " + systemEventIndex);
					...
					for (int iEvent = 0; iEvent < systemEvents.size(); iEvent++) {
						AceEvent priorEvent = (AceEvent) systemEvents
								.get(iEvent);
						if (!priorEvent.subtype.equals(event.subtype))
							continue;
	...
	private Datum corefFeatures(AceEvent priorEvent, AceEvent event,
		...
		// matching anchors
		if (lastMentionOfEvent.anchorText
				.equals(lastMentionPriorEvent.anchorText))
			d.addF("anchorMatch");
	...
	static String chXmlDir = home + "boost_ch_en_v2/";
	
	public void tag1(String fileList) throws IOException {
		BufferedReader reader = new BufferedReader(new FileReader(fileList));
		int docCount = 0;
		String currentDocPath;
		while ((currentDocPath = reader.readLine()) != null) {
			docCount++;
			System.out.println("\nProcessing file "+Integer.toString(docCount)+":"+currentDocPath);
			...
			String outputFile;
			AceDocument chAceDoc = null;
			if (currentDocPath.endsWith(".en.sgm")){
				outputFile = home + "boost_en_tagged/"
				+ currentDocPath.replaceFirst(".sgm", ".apf.xml");
				
			}
			else
				outputFile = home + "en_test_output/"
	...
	private void combineEvent(AceDocument enAceDoc,AceDocument chAceDoc){
		//ArrayList chEvents = chAceDoc.events;
		ArrayList chEvents = chAceDoc.events;
		for (int i = 0; i < chEvents.size(); i++) {
			AceEvent chEvent = (AceEvent) chEvents.get(i);
			if (!findEvent(enAceDoc,chEvent))
				enAceDoc.addEvent(chEvent);
		}
		
	}
	
	private boolean findEvent(AceDocument enAceDoc,AceEvent chEvent){
		ArrayList enEvents = enAceDoc.events;
		if (chEvent.mentions.size()==0)
			return true;
		AceEventMention m = chEvent.mentions.get(0);
		for (int i=0;i<enEvents.size();i++){
			AceEvent enEvent = (AceEvent) enEvents.get(i);
			AceEventMention m1 = enEvent.mentions.get(0);
			if (m1.anchorExtent.overlap(m.anchorExtent)){
				if (m1.confidence>=m.confidence)
					return true;
				else{
					enAceDoc.events.remove(enEvent);
					return false;
				}
			}
		}
	...
	public void eventCoref(AceDocument aceDoc, Document doc,
		...
		nextevent: for (int i = 0; i < events.size(); i++) {
			AceEvent event = (AceEvent) events.get(i);
	...
	static final String fileListTrain = home + "en_trainfilelist";
	static final String fileList = home + "filelist";
	...
	static String logFileName = home + "logfile";
	static String tempfileList1 = home + "enfilelist_en_sgm";
	...
	public static void main(String[] args) throws IOException {
		...
		for (int runs = 0; runs < 100; runs++) {
			FileWriter bw = new FileWriter(logFileName, true);
			bw.write("runs=" + Integer.toString(runs) + "\n");
			
			splitCorpus(fileList, 100, fileListTrain);

			ArrayList<String> v = new ArrayList<String>();
			BufferedReader reader = new BufferedReader(new FileReader(
					tempfileList1));
			String currentDocPath1;
			while ((currentDocPath1 = reader.readLine()) != null) {
				v.add(currentDocPath1);
			}
			...
			for (int i = 0; i <= 15; i += 20) {
				...
				if (trainPatterns) {
					if (trainArgModels) {
						et.argFeatureWriter = new PrintStream(
								new FileOutputStream(argFeatureFileName));
						et.roleFeatureWriter = new PrintStream(
								new FileOutputStream(roleFeatureFileName));
					}
					System.out.println("Trial:"+Integer.toString(runs)+"\tSample size="+Integer.toString(i));
					...
					if (trainArgModels) {
						...
						buildClassifierModel(argFeatureFileName,
								argModelFileName);
						buildClassifierModel(roleFeatureFileName,
								roleModelFileName);
					}
					if (savePatterns)
						et.save(eventPatternFile);
				}
				System.out.println("Training event model start...");
				if (trainEvModel) {
					trainEventModel(et);
				}
				System.out.println("Training event model end...");
				/*if (trainCorefModel) {
					trainCorefModel(et);
				}*/
				if (evaluate) {
					...
					System.out.println("tag test start...");
					evaluateEventTagger(et);
					System.out.println("tag test end...");
					EventEval2 ee = new EventEval2();
					ee.evaluate(fileListTest, home+"output/");
					bw.write(ee.TIP + "\t" + ee.TIR + "\t" + ee.TIF + "\t"
							+ ee.TCP + "\t" + ee.TCR + "\t" + ee.TCF + "\t"
							+ ee.AIP + "\t" + ee.AIR + "\t" + ee.AIF + "\t"
							+ ee.ACP + "\t" + ee.ACR + "\t" + ee.ACF + "\n");
				}
				if (i < 160) {
					FileWriter fw = new FileWriter(fileListTrain, true);
					...
					System.out.println("tag bitext start...");
					et.tag(newtestfileList);
					System.out.println("tag bitext end...");
				}
			}
			bw.close();
		}
	...
	public static void cotraining(ETCT et) throws IOException {
		if (trainPatterns) {
			if (trainArgModels) {
				et.argFeatureWriter = new PrintStream(
						new FileOutputStream(argFeatureFileName));
	...
	static String newtestfileList = home + "newtestfileList";
	
	static void splitCorpus(String corpus, int splitCount, String fileList1)
			throws IOException {
		int docCount = 495;
	...
	private static void trainEventModel(ETCT et) throws IOException {
		processOnlyOddDocuments = true;
		// set arg models
		et.argFeatureWriter = new PrintStream(new FileOutputStream(
				argFeatureHalfFileName));
		et.roleFeatureWriter = new PrintStream(new FileOutputStream(
		...
		// set ev model
		et.eventFeatureWriter = new PrintStream(new FileOutputStream(
	...
	private static void trainCorefModel(ETCT et) throws IOException {
		argModel = loadClassifierModel(argModelHalfFileName);
		roleModel = loadClassifierModel(roleModelHalfFileName);
		eventModel = loadClassifierModel(eventModelFileName);
		et.load(halfEventPatternFile);
		// set coref model
		et.corefFeatureWriter = new PrintStream(new FileOutputStream(
				corefFeatureFileName));
		// train coref model on even documents, using models from odd documents
		processOnlyEvenDocuments = true;
		et.train(fileListTrain, 3);
		/*
		 * et.train (fileListTrain1, 3); et.train (fileListTrain2, 3); et.train
		 * (fileListTrain3, 3); et.train (fileListTrain4, 3); et.train
		 * (fileListTrain5, 3); et.train (fileListTrain6, 3);
		 */
		// build coref model
	...
	private static void evaluateEventTagger(ETCT et) throws IOException {
		et.loadAllModels(aceModels);
