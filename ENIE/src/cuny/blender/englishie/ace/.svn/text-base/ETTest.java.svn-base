...
public class ETTest {
	...
	AceEvent eventAnchoredByConstituent(Annotation constit, Document doc,
		...
		log.append(patternMatched.toString()+"\n");
	...
	AceEvent pruneEvent(AceEvent event, Annotation constit, Document doc,
		...
		double eventProb = eventModel.eval(d.toArray())[eventModel
				.getIndex("event")];
		log.append("***P="+Double.toString(eventProb)+"\n");
		if (eventProb < EVENT_PROBABILITY_THRESHOLD){
			log.append("Lower than threshold...\n");
			return null;
		}
		else
		{
			log.append("Pass threshold...\n");
		}
	...
	AceEvent matchPatternSet(List patterns, Span anchorExtent, String anchor,
		...
		if (bestEvent==null){
			log.append("Anchor="+anchor+", Matched but arguments lower than 50%\n");
		}
			...
			for (int i = 0; i <= 160; i += 20) {
				...
				if (evaluate) {
					...
					BufferedWriter w3 = new BufferedWriter(new OutputStreamWriter(
							new FileOutputStream(home+"/log"+Integer.toString(i/20)+".html"), "UTF-8"));
					w3.flush();
	...
	private static void trainCorefModel(ETTest et) throws IOException {
		argModel = loadClassifierModel(argModelHalfFileName);
		roleModel = loadClassifierModel(roleModelHalfFileName);
	...
	private static void evaluateEventTagger(ETTest et) throws IOException {
