...
public class ETGraphCoref {
	...
	static double tcoef1 = 0.43;
	static double tcoef2 = 0.19;
	static double tcoef3 = 0.11;
	static double tcoef4 = 0.12;
	//static double tcoef5 = 0.12;

	static double acoef1 = 0.37;
	static double acoef2 = 0.14;
	...
	public DoubleMatrix2D csimFormula(Vector<AceEvent> v, Document doc,
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = i; j < n; j++) {
				...
				String anchor2 = EventPattern.normalizedAnchor(m2.anchorExtent,
						m2.anchorText, doc, relations);
				String str1= m1.anchorText.toLowerCase();
				String str2= m2.anchorText.toLowerCase();
				double anchorSim;
				PorterStemmer ps = new PorterStemmer();
				if (str1.equals(str2)) {
					anchorSim = tcoef1;
				} else {
					String stem1 = ps.stem(str1);
					String stem2 = ps.stem(str2);
					if (stem1.equals(stem2)) {
						anchorSim = tcoef2;
					} else {
						// quantized anchor similarity
				...
				for (int k = 0; k < m1.arguments.size(); k++) {
					String id1 = m1.arguments.get(k).value.getParent().id;
					String role1 = m1.arguments.get(k).role;

					for (int m = 0; m < m2.arguments.size(); m++) {
						count++;
						String id2 = m2.arguments.get(m).value.getParent().id;
						String role2 = m2.arguments.get(m).role;
						// role and id match
						if (id1.equals(id2) && role1.equals(role2)) {
							count1++;
							//break;
	...
	public DoubleMatrix2D csimFormula1(Vector<AceEvent> v, Document doc,
			SyntacticRelationSet relations) {
		int n = v.size();
		DoubleMatrix2D w;
		if (useSparseMatrix)
	...
	public static void main(String[] args) throws IOException {
		...
		//	Collections.shuffle(v);
			...
			//	splitCorpus(fold, v);
				...
				ETGraphCoref et = new ETGraphCoref();
				...
				if (evaluate) {
					...
					for (CLUSTER_THRESHOLD = 1.00; CLUSTER_THRESHOLD < 1.20; CLUSTER_THRESHOLD += 0.05) {
						System.out.println("CLUSTER_THRESHOLD= " + Double.toString(CLUSTER_THRESHOLD));
						evaluateEventTagger(et, 1);
						
						/*MUCCorefEval ee1 = new MUCCorefEval();
						ee1.evalCoref(fileListTest);
						
						bw11.write(Double.toString(ee1.precision) + "\t"
								+ Double.toString(ee1.recall) + "\t"
								+ Double.toString(ee1.F) + "\n");
						// bw.write("BCube\n");
						BCubeCorefEval ee2 = new BCubeCorefEval();
						ee2.evalCoref(fileListTest);
						bw21.write(Double.toString(ee2.precision) + "\t"
						...
						bw31.write(Double.toString(ee3.precision) + "\t"
								+ Double.toString(ee3.recall) + "\t"
								+ Double.toString(ee3.F) + "\n");
						
						/*evaluateEventTagger(et, 1);
						// bw1.write("COREF_THRESHOLD=" +
						// Double.toString(COREF_THRESHOLD) + "\n");
					...
					evaluateEventTagger(et, 2);
					// bw1.write("COREF_THRESHOLD=" +
					// Double.toString(COREF_THRESHOLD) + "\n");
					ee1 = new MUCCorefEval();
					ee1.evalCoref(fileListTest);
					// bw.write("MUC\n");
					bw13.write(Double.toString(ee1.precision) + "\t"
							+ Double.toString(ee1.recall) + "\t"
							+ Double.toString(ee1.F) + "\n");
					// bw.write("BCube\n");
					ee2 = new BCubeCorefEval();
					ee2.evalCoref(fileListTest);
					bw23.write(Double.toString(ee2.precision) + "\t"
							+ Double.toString(ee2.recall) + "\t"
							+ Double.toString(ee2.F) + "\n");
					ee3 = new CEAFCorefEval();
					ee3.evalCoref(fileListTest);
					bw33.write(Double.toString(ee3.precision) + "\t"
							+ Double.toString(ee3.recall) + "\t"
							+ Double.toString(ee3.F) + "\n");
				
					evaluateEventTagger(et, 3);
					...
					evaluateEventTagger(et, 4);
					
					ee1 = new MUCCorefEval();
					ee1.evalCoref(fileListTest);
				
					bw15.write(Double.toString(ee1.precision) + "\t"
						+ Double.toString(ee1.recall) + "\t"
						+ Double.toString(ee1.F) + "\n");
				// bw.write("BCube\n");
					ee2 = new BCubeCorefEval();
					ee2.evalCoref(fileListTest);
					bw25.write(Double.toString(ee2.precision) + "\t"
						+ Double.toString(ee2.recall) + "\t"
						+ Double.toString(ee2.F) + "\n");
					ee3 = new CEAFCorefEval();
					ee3.evalCoref(fileListTest);
					bw35.write(Double.toString(ee3.precision) + "\t"
							+ Double.toString(ee3.recall) + "\t"
							+ Double.toString(ee3.F) + "\n");*/
	...
	private static void evaluateEventTagger(ETGraphCoref et, int choice)
			throws IOException {
