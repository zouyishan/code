...
public class ETCTV2 {
	...
	static double EVENT_PROBABILITY_THRESHOLD = 0.1;// 0.80;
	...
	static final String home = "/home/zheng/Application/EngET/ct2/";
	...
	public void tag(String fileList) throws IOException {
		...
		while ((currentDocPath = reader.readLine()) != null) {
			...
			if (currentDocPath.endsWith(".en.sgm")){
				outputFile = home + "sources/"
				+ currentDocPath.replaceFirst(".sgm", ".apf.xml");
				String chXmlFile = chXmlDir
					+ currentDocPath.replaceFirst(".sgm", ".apf.xml");
				chAceDoc = new AceDocument(textFile, chXmlFile);
			}
			else
				outputFile = home + "output/"
			...
			for (int i = 0; i <= 160; i += 20) {
				...
				if (evaluate) {
					...
					BufferedWriter w3 = new BufferedWriter(new OutputStreamWriter(
							new FileOutputStream(home+"log/log"+Integer.toString(i/20)+".html"), "UTF-8"));
					w3.flush();
					w3.write(ee.log.toString());
					w3.close();
	...
	private static void trainCorefModel(ETCTV2 et) throws IOException {
		argModel = loadClassifierModel(argModelHalfFileName);
		roleModel = loadClassifierModel(roleModelHalfFileName);
	...
	private static void evaluateEventTagger(ETCTV2 et) throws IOException {
