...
public class Ace {
	...
	public static boolean monocase = true;
	...
	static AceEventCorefGraph eventTagger = null;
	...
	public static void main (String[] args) throws IOException {
		...
		if (eventModelsDir != null) {
			eventTagger = new AceEventCorefGraph();
			eventTagger.useParser = useParser;
			eventTagger.usePA = (glarfDir != null);
			eventTagger.triplesDir = glarfDir;
			
			eventTagger.type="system";
			eventTagger.CLUSTER_THRESHOLD=0.85;
			eventTagger.simMeasure=1;
	...
	static void processFile (String currentDocPath) throws IOException{
		...
		// read document
		doc = new ExternalDocument("sgml", docDir + currentDocPath);
		if (perfectMentions) {
		...
		// build events
		if (eventTagger != null)
			eventTagger.tag(doc, aceDoc, currentDocPathBase, docId,1);
