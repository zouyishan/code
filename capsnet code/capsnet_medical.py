...
recon_dim = 562
n_c = 20
resultDir = './medical/result_test03/'
dataPath = 'medical_20_minc5.pkl'
...
def CapsNet(input_shape, n_class, num_routing):
    ...
    lstm = layers.LSTM(128, name='lstm', return_sequences=True, dropout=0.2,
                       recurrent_dropout=0.2)(embed)
    ...
    embed_conv = layers.Conv1D(filters=1, kernel_size=128, strides=128, padding='valid',
...
def get_model(isTrain=True):
    ...
    x, y = read_data.load_pickle_data(dataPath)
    y = np.eye(n_c)[y]
    # x_train = x[:6274]
    # y_train = y[:6274]
    # x_v = x[6274:]
...
def test_model(model, test_x, test_y):
    print('predicting...')
    y_pre, x_recon = model.predict([test_x, np.eye(n_c)[test_y]])
    y_pre = y_pre.tolist()
    y_pre = [l.index(max(l)) for l in y_pre]

    cats = [i for i in range(n_c)]
    p = []
    r = []
    f1 = []
    tp_sum = 0
