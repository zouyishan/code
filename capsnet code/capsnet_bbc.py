...
def CapsNet(input_shape, n_class, num_routing):
    ...
    x_recon = layers.Dense(2048, activation='relu')(x_recon)
...
def get_model(isTrain=True):
    ...
    x, y = read_data.load_pickle_data(read_data.bbc_pkl_path)
    y = np.eye(5)[y]
    x_train = x[:1600]
    y_train = y[:1600]
    x_v = x[1600:1780]
...
def test_model(model, test_x, test_y):
    print('predicting...')
    y_pre, x_recon = model.predict([test_x, np.eye(5)[test_y]])
    y_pre = y_pre.tolist()
    y_pre = [l.index(max(l)) for l in y_pre]

    cats = [i for i in range(5)]
    p = []
    r = []
    f1 = []
    tp_sum = 0
